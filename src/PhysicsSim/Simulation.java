...
public class Simulation {
    ...
    // and softbodies as the negative integers -1 is the wall.
        ...
        if (validDemo()) {
            ...
            switch(demoID) {
                ...
                case(6): {
                    ...
                    addRigidbodyPolygon(5, 15.0, new double[]{250.0, 150.0, 0.0, 150.0, 0.0, 90.0, 0.0, 0.0}, 1.0, Color.blue);
                ...
                case(7): {
                    ...
                    addSpringSoftbody(new double[]{200.0, 475.0, 400.0, 100.0}, new double[]{400.0, 400.0, 325.0, 335.0}, new double[]{0.0, 0.0, 0.0, 90.0},
                            500.0, Color.blue, 1000.0, 15.0, 5.0 / Math.sqrt(2));
                ...
                case(9): {
                    ...
                    //addRigidbodyPolygon(new double[]{-16, 16, 16, -16}, new double[]{-32, -32, 32, 32}, new double[]{-250.0, 220.0, 0.0, 0.0, 0.0, 90.0, 0.0, 0.0}, 100, Color.blue);
                    addObstaclePolygon(new double[]{-500.0, 1000.0, 1000.0, -500.0}, new double[]{400.0, 400.0, 500.0, 500.0}, 10.0, Color.green);
                    getObject(0).addStandardControllerBundle();
                    getObject(1).setMaterial("Ground1");
                    ...
                    getObject(2).setMaterial("Ground2");
                ...
                case(11): {
                    ...
                    for (int i = 1; i <= 4; i++) {
                        addRigidbodyPolygon(new double[]{-25.0, 25.0, 25.0, -25.0}, new double[]{-25.0, -25.0, 25.0, 25.0}, new double[]{27.0 + 150.0 * (i - 1), 400.0, 0.0, 0.0, 0.0, 90.0, 0.0, 0.0}, 10.0, Color.blue);
                        addRigidbodyPolygon(new double[]{20.0, -10.0, -10.0}, new double[]{0.0, 10.0 * Math.sqrt(3.0), -10.0 * Math.sqrt(3.0)}, new double[]{27.0 + 150.0 * (i - 1), 50.0, 0.0, 0.0, 0.0, 90.0, 0.0, 0.0}, 20.0, Color.yellow);
                        Rigidbody base = Rigidbody.get(2 * i - 2);
                        Rigidbody top = Rigidbody.get(2 * i - 1);
                        switch (i) {
                            //distance
                            case 1: {
                    ...
                    for (int i = 1; i <= 4; i++) {
                        addObstaclePolygon(new double[]{102.0 + 130.0 * (i - 1), 102.0 + 130.0 * (i - 1), 102.0 + 130.0 * (i - 1) + 5.0, 102.0 + 130.0 * (i - 1) + 5.0}, new double[]{0.0, 460.0, 460.0, 0.0},1.0, Color.green);
                    }
    ...
    public void addHitboxPolygon(double[] x, double[] y, double[] movingMotion, Color color, String name) {
        if (showCreationInfo) System.out.println("Hitbox " + hitboxes.size() + ": 'Rigidbody' " + Rigidbody.num);
        double[] motion = new double[]{0.0, 0.0, movingMotion[0], movingMotion[1], movingMotion[2], movingMotion[3], movingMotion[4], movingMotion[5]};
    ...
    public Hitbox getHitbox(String name) {
        for (Hitbox hitbox : hitboxes) {
            if (hitbox.name.equals(name)) return hitbox;
        }
        return null;
    }
    ...
    public void describeAllObjects() {
        ...
        for (PhysicsObject physicsObject : physicsObjects) {
            if (physicsObject.getType().equals("Rigidbody") && physicsObject.rigidbody.parentSoftbody == -1) {
                System.out.print("Physics Object " + i + ": ");
                Rigidbody rigidbody = physicsObject.rigidbody;
                if (rigidbody.isMovable()) System.out.print("Dynamic");
                else System.out.print("Obstacle");
                System.out.print(" 'Rigidbody' " + physicsObject.getGeometryType() + " ");
                if (physicsObject.getGeometryType().equals("Polygon")) {
                    boolean face = false;
                    for (Triangle triangle : ((Polygon) rigidbody.geometry).triangles) {
                        if (triangle.doesExist() && triangle.partOfFace) {
                            face = true;
                            break;
                        }
                    }
                    if (face) System.out.print("(Face) ");
                }
                if (rigidbody.isAttached) System.out.print(" (Has Joints) ");
                System.out.println(rigidbody.ID);
                System.out.print("Radius: " + physicsObject.rigidbody.geometry.getLargestDistance());
                System.out.println(", Area: " + rigidbody.getArea() + ", Moment of Inertia: " + rigidbody.getInertia() + ", Mass: " + rigidbody.getMass()
                        + ", Difference against air density: " + ((rigidbody.getMass() / rigidbody.getArea()) - AIR_DENSITY));
                if (physicsObject.getGeometryType().equals("Polygon")) System.out.println(((Polygon) rigidbody.geometry).getTriangulationCategorization());
                System.out.println();

            }
            if (physicsObject.getType().equals("Softbody")) {
                System.out.print("Physics Object " + i + ": ");
                Softbody softbody = physicsObject.softbody;
                System.out.println("'Softbody' " + softbody.ID);
                System.out.println("Size: " + softbody.size() + " members and " + softbody.boundarySize() +
                        " boundary members with " + (softbody.mass / softbody.size()) + " mass per member.");
                switch (softbody.getType()) {
                    case SoftbodyType.PressureSpring: {
                        System.out.println("Initial Pressure: " + softbody.getInitialPressure() + " | Type: PressureSpring.");
                        break;
                    }
                    ...
                    case SoftbodyType.ShapeMatchSolid: {
                        System.out.println("Shape-Matching -   Spring Strength: " + softbody.SHAPE_MATCH_STRENGTH + ", Spring Damping: " + softbody.SHAPE_MATCH_DAMPING);
                    }
                    case SoftbodyType.SpringMass: {
                        ...
                        System.out.println("Spring Strength: " + softbody.getMember(0).getSoftbodyAttachmentJoint(0).SPRING_CONSTANT +
                                ", Spring Damping: " + softbody.getMember(0).getSoftbodyAttachmentJoint(0).SPRING_DAMPING + " | Spring-Mass");
                        break;
                    }
    ...
    public void describeAllHitboxes() {
        int i = 0;
        for (Hitbox hitbox : hitboxes) {
            if (hitbox.getType().equals("Rigidbody")) {
                System.out.print("Hitbox " + i + ": ");
                Rigidbody rigidbody = hitbox.rigidbody;
                System.out.print(hitbox.getGeometryType() + " " + rigidbody.ID);
                switch (hitbox.getGeometryType()) {
                    case "Polygon": {
                        Polygon polygon = (Polygon) hitbox.rigidbody.geometry;
                        System.out.print("Points: ");
                        double x = rigidbody.getPosX();
                        double y = rigidbody.getPosY();
                        for (int j = 0; j < polygon.getNumPoints(); j++) {
                            System.out.print("{" + (polygon.getXPoints(j) + x) + ", " + (polygon.getYPoints(j) + y) + "}");
                            if (j < polygon.getNumPoints() - 1) System.out.print(",  ");
                        }
                        break;
                    }
                    case "Circle": {
                        System.out.println("Center: {" + rigidbody.getPosX() + ", " + rigidbody.getPosY() + "}, Radius: " + rigidbody.geometry.getLargestDistance());
                        break;
                    }
                    default: {
        ...
        System.out.println("Size: " + hitboxes.size() + " hitboxes.");
    }
